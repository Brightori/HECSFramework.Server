...
namespace HECSFrameWork
{
    public partial class CodeGenerator
    {
        ...
        public List<(string name, string content)> GetSerializationResolvers()
        {
            ...
            var networkcomponentType = typeof(INetworkComponent);

            var asses = AppDomain.CurrentDomain.GetAssemblies().SelectMany(s => s.GetTypes());

            commands = asses.Where(p => componentType.IsAssignableFrom(p) && !p.IsGenericType && !p.IsInterface && !p.IsAbstract).ToList();

            foreach (var c in commands)
            {
                if (networkcomponentType.IsAssignableFrom(c))
                {
        ...
        private ISyntax GetResolver(Type c)
        {
            ...
            tree.Add(new NameSpaceSyntax("HECSFrameWork"));
            ...
            if (typeof(IAfterSyncComponent).IsAssignableFrom(c))
            {
        ...
        public string GetResolverMap()
        {
            ...
            tree.Add(new UsingSyntax("HECSFrameWork"));
            tree.Add(new UsingSyntax("HECSFrameWork.Components", 1));
